...
void addHeap(DynArr *heap, TYPE val)
{
	...
	while (cur != 0) {
		...
		//If the new node's val is less than its parent nodes val, swap them
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if (rightIdx < max) { //If there are 2 children
		...
		//If the smaller child is less than the parent, swap
	...
	else if (leftIdx < max) { //If there is 1 child
		//if the child is less than the parent, swap
...
void removeMinHeap(DynArr *heap)
{
	...
	//Replace the first nodes value with the last
