...
void ResetArm()
{
  ...
  int ret = WaitForTouchPressedResetArm(1);
  //stop the motors.
  motor[ArmMotor2] = 0;
  motor[ArmMotor1] = 0;
  //reset motor encoder values to 0 only if it was normal reset all the way to touch pressed
  if(ret == 1) // it is normal reset
  {
...
void ArmToPosition(unsigned int Position)
{
...

  {
    ...
    unsigned int trayPos = TRAY_RESET_POS - (nMotorEncoder[ArmMotor2]/30);

		  	    // check to see if tray has reached top limit.
		        // rotate only if it is not reached the top limit.
		    	  //	if (trayPos > 100)
		    		//ServoRotateSmooth(tray, trayPos  );
    RotateTraySmooth(trayPos);
